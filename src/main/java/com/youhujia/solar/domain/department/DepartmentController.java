...
public class DepartmentController extends BaseController {
    ...
    public Solar.CreateOrUpdateDepartmentDTO createDepartmentForZhuShou(@RequestBody Solar.LBSCreateDepartmentOption option) {

        try {
            return departmentBO.lbsCreateDepartment(option);
        } catch (Exception e) {
            return handleException(a -> Solar.CreateOrUpdateDepartmentDTO.newBuilder().setResult(a).build(), e);
        }
    }
    ...
    @RequestMapping(value = "/manager-department", method = RequestMethod.POST)
    public Solar.ManagerDepartmentDTO managerCreateDepartment(@RequestBody Solar.CreateOrUpdateDepartmentOption option) {

        try {
            return departmentBO.managerCreateDepartment(option);
        } catch (Exception e) {
            return handleException(a -> Solar.ManagerDepartmentDTO.newBuilder().setResult(a).build(), e);
        }
    }

    @RequestMapping(value = "/manager-department/{departmentId}", method = RequestMethod.PATCH)
    public Solar.ManagerDepartmentDTO managerUpdateDepartment(@PathVariable("departmentId") Long departmentId, @RequestBody Solar.CreateOrUpdateDepartmentOption option) {

        try {
            return departmentBO.managerUpdateDepartment(departmentId, option);
        } catch (Exception e) {
            return handleException(a -> Solar.ManagerDepartmentDTO.newBuilder().setResult(a).build(), e);
        }
    }
    ...
    @RequestMapping(value = "/manager-department/{departmentId}", method = RequestMethod.GET)
    public Solar.ManagerDepartmentDTO managerGetDepartmentById(@PathVariable("departmentId") Long departmentId) {

        try {
            return departmentBO.managerGetDepartmentById(departmentId);
        } catch (Exception e) {
            return handleException(a -> Solar.ManagerDepartmentDTO.newBuilder().setResult(a).build(), e);
        }
    }
